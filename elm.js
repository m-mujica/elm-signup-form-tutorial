...
Elm.SignupForm.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var viewUsernameErrors = function (model) {
   ...
   var withUsernameTaken = F2(function (isTaken,model) {
      ...
      var newErrors = _U.update(currentErrors,
   ...
   var UsernameAvailable = {ctor: "UsernameAvailable"};
   var UsernameTaken = {ctor: "UsernameTaken"};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Validate": var url = A2($Basics._op["++"],
           "https://api.github.com/users/",
           model.username);
           var request = A2($Http.get,
           $Json$Decode.succeed(UsernameTaken),
           url);
           var safeRequest = A2($Task.onError,
           request,
           function (_p1) {
              return $Task.succeed(UsernameAvailable);
           });
           return {ctor: "_Tuple2"
                  ,_0: _U.update(model,{errors: getErrors(model)})
                  ,_1: $Effects.task(safeRequest)};
         case "SetUsername": return {ctor: "_Tuple2"
                                    ,_0: _U.update(model,{username: _p0._0})
                                    ,_1: $Effects.none};
         case "SetPassword": return {ctor: "_Tuple2"
                                    ,_0: _U.update(model,{password: _p0._0})
                                    ,_1: $Effects.none};
         case "UsernameAvailable": return {ctor: "_Tuple2"
                                          ,_0: A2(withUsernameTaken,false,model)
                                          ,_1: $Effects.none};
         default: return {ctor: "_Tuple2"
                         ,_0: A2(withUsernameTaken,true,model)
                         ,_1: $Effects.none};}
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.h1,
              ...
              _U.list([$Html$Attributes.id("password-field")
                      ...
                      $Html$Events.targetValue,
                      function (str) {
                         return A2($Signal.message,address,SetPassword(str));
                      })]),
              _U.list([]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("validation-error")]),
              ...
              _U.list([$Html$Attributes.$class("signup-button")
                      ,A2($Html$Events.onClick,address,Validate)]),
              _U.list([$Html.text("Sign Up!")]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("validation-error")]),
              _U.list([$Html.text(viewUsernameErrors(model))]))]));
   });
   var initialErrors = {username: ""
                       ,password: ""
                       ,usernameTaken: false};
   ...
   var Model = F3(function (a,b,c) {
      return {username: a,password: b,errors: c};
   });
   var Errors = F3(function (a,b,c) {
      return {username: a,password: b,usernameTaken: c};
   });
